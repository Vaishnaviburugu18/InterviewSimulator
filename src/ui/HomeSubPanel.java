...
public class HomeSubPanel extends JPanel {
    ...
    private void addModeCard(JPanel grid, String title, String desc, Color accent, String mode) {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (mode.equals("Company")) {
                    ...
                    String choice = (String) JOptionPane.showInputDialog(
                            frame, "Select Company:", "Company Test",
                            JOptionPane.PLAIN_MESSAGE, null, companies, companies[0]);
                    if (choice != null) frame.startQuiz(user, null, "Company", choice);
                } else {
                    // Pick a domain
                    String[] domains = QuizService.ALL_DOMAINS;
                    String domain = (String) JOptionPane.showInputDialog(
                            frame, "Select Domain:", "Choose Domain",
                            JOptionPane.PLAIN_MESSAGE, null, domains, domains[0]);
    ...
    public void refresh(User u) {
        ...
        try {
            ...
            statsRow.add(statCard("🧪 Tests Taken", String.valueOf(tests), new Color(99, 102, 241)));
            statsRow.add(statCard("🏆 Best Score",  best + "%",            new Color(16, 185, 129)));
            statsRow.add(statCard("📊 Average",      avg + "%",             new Color(245, 158, 11)));
            statsRow.add(statCard("⭐ XP / Level",   xp + " / Lv." + level, new Color(139, 92, 246)));
            ...
            statsRow.add(statCard("Error", "DB", Color.RED));
        }
    ...
    private JPanel activityRow(String topic, int score, String mode, java.sql.Timestamp date) {
        ...
        mid.setOpaque(false);
        JLabel topicL = new JLabel(QuizService.iconFor(topic) + "  " + topic);
